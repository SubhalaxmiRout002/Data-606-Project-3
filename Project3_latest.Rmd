...
#data <-  data %>% filter(!is.na(salary_lower_range))
#unique(unlist(strsplit(data$hard_skills_2,",")))
#unique(unlist(strsplit(unique(data$soft_skills_2),",")))
```

```{r}
hard_skill_count <- unlist(lapply(strsplit(data$hard_skills_2,","),function (x) paste(x)))

hard_skill_count <-  table(hard_skill_count) %>% as.data.frame()

hard_skill_count$hard_skill_count <- as.character(hard_skill_count$hard_skill_count)
hard_skill_count$Freq <- as.numeric(hard_skill_count$Freq)
hard_skill_count$Freq <- hard_skill_count$Freq[order(-hard_skill_count$Freq)]
datatable(hard_skill_count)
```
