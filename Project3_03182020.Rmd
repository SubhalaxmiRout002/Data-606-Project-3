...
data %>% dplyr::select  (one_of(c("hard_skills_2", "hard_skill_groupings"))) %>% head(4)
...
data %>% dplyr::select  (one_of(c("soft_skills_2", "soft_skill_groupings"))) %>% head(4)
...
length(unique(unlist(strsplit(data$soft_skills_2, ","))))

length(unique(unlist(strsplit(data$hard_skills_2, ","))))
...
# filtering out exception cases 
hard_skills_count_df_final1 <- hard_skills_count_df_final %>% filter (!(Var1  == "r" | Var1 == "snr signals" | Var1 == "AI/ML&Algorithmsscript" | Var1 == "Visualizationss" | Var1 == "Visualizationsview"))


```

```{r fig.width=8, fig.height= 4}
  ggplot(data = hard_skills_count_df_final1) +
  aes(x = reorder(Var1, Frequency_Percent), y = Frequency_Percent) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(Frequency_Percent, "%")), hjust = -.15) +
...
data2 <- data1 %>% dplyr::select (city_state, hard_skill_groupings_2, soft_skill_groupings_2)
head(data2, 4)
...
corpus.city <- tm::VCorpus(tm::VectorSource(cities))
#list(c("Teamwork", "Problem-Solving","Creativity", "Work Ethic", "Interpersonal Skills", "Time Management", "Leadership", "Communication Skills"))
