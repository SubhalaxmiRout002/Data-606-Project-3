...
# %>%group_by( paste(location, sep = " ", state)) 
...
as.data.frame(table(data2$city_state)) %>% arrange(desc(Freq))
# 1             New York NY   59
# 2          Los Angeles CA   41
...
ny <- data2[data2$city_state == "New York NY", 1]
la <- data2[data2$city_state == "Los Angeles CA", 1] 
sf <- data2[data2$city_state == "San Francisco CA", 1] 
chi <- data2[data2$city_state == "Chicago IL", 1] 
sd <- data2[data2$city_state == "San Diego CA", 1] 
bos <- data2[data2$city_state == "Boston MA", 1] 
wdc <- data2[data2$city_state == "Washington DC", 1] 
sc <- data2[data2$city_state == "Santa Clara CA", 1]
den <- data2[data2$city_state == "Denver CO", 1 ] 
sea <- data2[data2$city_state == "Seattle WA", 1]

cities <- c(ny, la, sf, chi, sd, bos, wdc, sc, den, sea)

corpus.city <- VCorpus(VectorSource(cities))
#list("Teamwork", "Problem-Solving","Creativity", "Work Ethic", "Interpersonal Skills", "Time Management", "Leadership", "Communication Skills")
...
data2$soft_skill_groupings_3 <- mapvalues(data2.city,
                                          from = 1:10,
                                          to = c("NY", "LA", "SF",
                                                 "CHI", "SD", "BOS",
                                                 "WDC", "SC", "DEN", "SEA"
